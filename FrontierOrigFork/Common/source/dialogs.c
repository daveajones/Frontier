...
void boldenbutton (DialogPtr pdialog, short itemnumber) {
	...
	*/
	...
	pushport (GetDialogPort(pdialog));
...
void positiondialogwindow (DialogPtr pdialog) {
	...
	GetPortBounds(GetDialogPort(pdialog), &rdialog);
	...
	movewindow(GetDialogWindow(pdialog), h, v);
...
pascal boolean modaldialogcallback (DialogPtr pdialog, EventRecord *ev, short *item) {
	...
	switch (whatevent) {
		...
		case updateEvt:
			...
			QDFlushPortBuffer(GetDialogPort(pdialog), nil);	
			break;
