...
static void runmodeupdate (void) {
	...
	pushmacport (GetWindowPort(w));
	if ((**hc).fullupdate) {
		Rect windBounds;
		GetPortBounds(GetWindowPort(w), &windBounds);
	...
	//Changed to Opaque call for Carbon
		...
		visRgn = GetPortVisibleRegion(GetWindowPort(w), visRgn);
...
static void filterevent (EventRecord *ev) {
	...
	pushmacport (GetWindowPort((**iowadata).drawwindow));
...
static void frameedittextrect (Rect r, WindowPtr w) {
	...
	*/
	...
	pendrawline (r.right, r.bottom); /*draw bottom of box*/
		...
		Rect rbackground;
		//short depth;
		GetPortBounds (GetWindowPort (w), &rwindow);
...
boolean runModalCard (Handle hpackedcard, boolean flscriptedcard, short top, short pLeft, tycardeventcallback pCallback) {
	...
	*/
	...
	SetPort (GetWindowPort(modalwindow));
...
boolean runModelessCard (Handle hpackedcard, boolean flscriptedcard, short top, short pLeft, tycardeventcallback pCallback) {
	...
	*/
	...
	SetPort (GetWindowPort(modelesswindow));
